import type { Messages } from "./tr";

const fr: Messages = {
  meta: {
    title: "Thailand Villa Rent — Locations en Thaïlande",
    description:
      "Hébergements sélectionnés en Thaïlande. Détails complets, distances et équipements pour Phuket, Koh Samui et plus.",
  },
  nav: { stays: "Hébergements", destinations: "Lieux à visiter" },
  home: {
    eyebrow: "À travers la Thaïlande",
    title: "Votre séjour idéal en Thaïlande",
    subtitle:
      "Hébergements sélectionnés à Phuket et Koh Samui. Distances, équipements, politiques et attractions à proximité.",
    cta: "Voir tous les hébergements",
    featured: "Hébergements en vedette",
    featuredHint: "Annonces réelles issues de Trip.com et sources officielles",
    viewAll: "Tout voir",
  },
  stays: {
    title: "Hébergements",
    subtitle: "5 hébergements sélectionnés à Phuket et Koh Samui avec tous les détails.",
    rooms: "chambres",
    fromPrice: "À partir de ~${price} USD/nuit",
  },
  regions: { phuket: "Phuket", "koh-samui": "Koh Samui" },
  airports: { hkt: "Aéroport international de Phuket", usm: "Aéroport international de Samui" },
  detail: {
    back: "Tous les hébergements",
    overview: "Aperçu",
    rooms: "Chambres",
    amenities: "Services et équipements",
    popular: "Équipements populaires",
    allAmenities: "Tous les équipements",
    policies: "Politiques",
    checkIn: "Arrivée",
    checkOut: "Départ",
    children: "Enfants",
    cribs: "Berceaux et lits supplémentaires",
    breakfast: "Petit-déjeuner",
    deposit: "Caution",
    pets: "Animaux",
    age: "Âge minimum",
    quietHours: "Heures de silence",
    payment: "Paiement",
    faq: "Questions fréquentes",
    location: "Emplacement et distances",
    airport: "Aéroport le plus proche",
    landmarks: "Points d'intérêt",
    beaches: "Plages et temps de trajet",
    showOnMap: "Voir sur la carte",
    opened: "Ouverture",
    renovated: "Rénovation",
    roomCount: "Nombre de chambres",
    min: "min",
    distanceM: "{m} m",
    distanceKm: "{km} km",
  },
  footer: {
    tagline: "Locations en Thaïlande",
    copyright: "© 2026 Thailand Villa Rent",
  },
};

export default fr;
